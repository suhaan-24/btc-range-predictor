...
├── backtest_results.jsonl   # Per-bar backtest output (git-ignored, generated locally)
└── backtest_metrics.json    # Summary metrics shown on dashboard (git-ignored)
```
...
python backtest.py
```
Outputs `backtest_results.jsonl` and `backtest_metrics.json` — the metrics file is read by the dashboard.
